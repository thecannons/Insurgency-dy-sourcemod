...
void UpdateRespawnCvars()
{
	...
	if (g_isHunt == 1)
	{
		...
		g_iRespawn_lives_team_ins = ((g_iRespawn_lives_team_ins * secTeamCount) / 5);
	}
}
...
void RespawnPlayer(client, target)
{
	...
	if(IsClientInGame(target) && !IsClientTimingOut(target) && ((g_isHunt == 1 && g_client_last_classstring[target][0])) && playerPickSquad[target] == 1 && !IsPlayerAlive(target) && team == TEAM_1)
	{
		// Write a log
...
public Action:Event_ControlPointCaptured_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_checkStaticAmtCntr = GetConVarInt(sm_respawn_check_static_enemy_counter);
	// Return if conquer
	if (g_isConquer == 1 || g_isHunt == 1) return Plugin_Continue;
