		...
		if (g_iReinforceTime <= 0)
		{
			...
			if (g_iRemaining_lives_team_ins > 0)
			{
				...
				if (g_iRemaining_lives_team_ins < (g_iRespawn_lives_team_ins / g_iReinforce_Mult) + g_iReinforce_Mult_Base)
				{
					...
					g_iReinforceTime = g_iReinforceTimeSubsequent;
					if (g_isHunt == 1)
						 g_iReinforceTime = g_iReinforceTimeSubsequent * g_iReinforce_Mult;
				...
				else
				{
					...
					g_iReinforceTime = GetConVarInt(sm_respawn_reinforce_time);
					if (g_isHunt == 1)
						 g_iReinforceTime = g_iReinforceTime * g_iReinforce_Mult;
					//if (g_huntCacheDestroyed == true && g_isHunt == 1)
					//	g_iReinforceTime = g_iReinforceTime + g_huntReinforceCacheAdd;
				}
...
public Action:Timer_AIDirector_Main(Handle:timer, any:data)
{
	...
	if (g_AIDir_AnnounceCounter >= g_AIDir_AnnounceTrig)
	{
		g_AIDir_AnnounceCounter = 0;
		g_AIDir_AnnounceTrig = 5;
		PrintToServer("[AI_DIRECTOR] STATUS: %i", g_AIDir_TeamStatus);
	}
...
public AI_Director_SetDifficulty(g_AIDir_TeamStatus, g_AIDir_TeamStatus_max)
{
	...
	new cvar;
	// < 25%
	if (g_AIDir_TeamStatus < (g_AIDir_TeamStatus_max / 4))
	{
		//Set Reinforce Time
		cvar = FindConVar("sm_respawn_reinforce_time");
		SetConVarInt(cvar, (g_AIDir_ReinforceTimer_Orig + 20), true, false);
		cvar = FindConVar("sm_respawn_reinforce_time_subsequent");
		SetConVarInt(cvar, (g_AIDir_ReinforceTimer_SubOrig + 20), true, false);
	...
	else if (g_AIDir_TeamStatus >= (g_AIDir_TeamStatus_max / 4) && g_AIDir_TeamStatus < (g_AIDir_TeamStatus_max / 2))
	{
		//Set Reinforce Time
		cvar = FindConVar("sm_respawn_reinforce_time");
		SetConVarInt(cvar, (g_AIDir_ReinforceTimer_Orig), true, false);
		cvar = FindConVar("sm_respawn_reinforce_time_subsequent");
		SetConVarInt(cvar, (g_AIDir_ReinforceTimer_SubOrig), true, false);
	...
	else if (g_AIDir_TeamStatus >= (g_AIDir_TeamStatus_max / 2) && g_AIDir_TeamStatus < ((g_AIDir_TeamStatus_max / 4) * 3))
	{
		//Set Reinforce Time
		cvar = FindConVar("sm_respawn_reinforce_time");
		SetConVarInt(cvar, (g_AIDir_ReinforceTimer_Orig - 10), true, false);
		cvar = FindConVar("sm_respawn_reinforce_time_subsequent");
		SetConVarInt(cvar, (g_AIDir_ReinforceTimer_SubOrig - 10), true, false);
