...
new
	...
	bool:g_bot_isSpawnedAlive[MAXPLAYERS+1] = false,
	bool:g_botSpecial_isSpawning[MAXPLAYERS+1] = true,
	bool:g_bot_OverFlowKilled[MAXPLAYERS+1] = false,
...
new
	...
	//Grab directly from theater (5 jugs, 4 bombers, All others 22) 3 scouts in standard for now.
	g_BotStdMax = 22,
	g_BotFighterMax = 5,
	g_BotStrikerMax = 6,
	g_BotScoutMax = 3,
	...
	g_BotStdAliveCnt = 0,
	g_BotFighterAliveCnt = 0,
	g_BotStrikerAliveCnt = 0,
	g_BotScoutAliveCnt = 0,
	g_BotSapperAliveCnt = 0,
	g_BotJuggerAliveCnt = 0,
	g_BotBomberAliveCnt = 0,

	//Starting Template of bots AI Director uses (determine by function)
	g_bots_std = 22,
	//g_bots_light = 2,
	g_bots_jug = 0,
	g_bots_bomb = 0,
	//This is our version of ins_bots_min - ins_bots_max
	g_bots_spawn_max = 0,
...
public OnPluginStart()
{
	...
	HookEvent("player_spawn", Event_SpawnPre);
...
void CalculateBotSpawnTemplate()
{
	...
	g_bots_jug = 0;
	g_bots_bomb = 0;
	//Determine specialized bots
	int secTeamCount = GetTeamSecCount();
	if (secTeamCount > 5 && secTeamCount <= 8)
	{
		if (g_AIDir_TeamStatus < 60)
		{	
			g_bots_bomb = 1;
			g_bots_jug = 1;
		}
		else if (g_AIDir_TeamStatus >= 60)
		{
			g_bots_bomb = 1;
			g_bots_jug = 2;
		}
		int fRandomInt = GetRandomInt(0, 10);
		if (fRandomInt <= 3)
		{	
			g_bots_bomb = 2;
			g_bots_jug = 2;
	...
	else if (secTeamCount > 8 && secTeamCount <= 14)
	{
		if (g_AIDir_TeamStatus < 60)
		{	
			g_bots_bomb = 2;
	...
	int tCalcBotMax = g_bots_spawn_max;
	tCalcBotMax -= (g_bots_bomb + g_bots_jug);
	PrintToServer("tCalcBotMax %d, g_bots_spawn_max %d", tCalcBotMax, g_bots_spawn_max);
	g_bots_std = tCalcBotMax;
}
...
void UpdateRespawnCvars()
{
	...
	/*
	 		DY BOT SPAWNING SYSTEM - Calculate spawn amounts and template. - START
	*/
	//Determine allowed number of bots to spawn based on player count
	new tSecTeamCount = GetTeamSecCount();
	g_bots_spawn_max = 6;
	if (tSecTeamCount <= 8)
			g_bots_spawn_max += (tSecTeamCount);
	else if (tSecTeamCount > 8 && tSecTeamCount <= 14)
			g_bots_spawn_max += (tSecTeamCount + 1);
	else if (tSecTeamCount > 14)
			g_bots_spawn_max += (tSecTeamCount + 2);

	if (g_bots_spawn_max > 26)
		g_bots_spawn_max = 26;

	CalculateBotSpawnTemplate();
	calcBotAlive();
	/*
	 		DY BOT SPAWNING SYSTEM - Calculate spawn amounts and template. - END
	*/
...
#####################################################################
#####################################################################
# Wave Based Spawning Timer START ############################
# Wave Based Spawning Timer START ############################
#####################################################################
#####################################################################
#####################################################################
...
public Action:Timer_WaveSpawning(Handle:Timer)
{
	...
	//Wave spawning for players only tracks g_secWave_Timer and sync's it with players individual timers.
	//Its a simple hack to simulate wave spawning without re-writing too much.  Bots are different.
	...
	PrintToServer("[DY_SPAWN] MAXBOTS: %i | JugAlive %d | BombAlive: %d | StdAlive: %d", g_bots_spawn_max, g_BotJuggerAliveCnt, g_BotBomberAliveCnt, g_BotStdAliveCnt);
	PrintToServer("[DY_SPAWN] MAXBOTS: %i | g_BotJuggMax %d | g_BotBombMax: %d | g_BotStdMax: %d", g_bots_spawn_max, g_BotJuggMax, g_BotBombMax, g_BotStdMax);
	PrintToServer("[DY_SPAWN] MAXBOTS: %i | g_bots_jug %d | g_bots_bomb: %d | g_bots_std: %d", g_bots_spawn_max, g_bots_jug, g_bots_bomb, g_bots_std);

	//Bots custom wave spawns
	if (g_respawn_mode_team_ins)
	{
		int insTeamAliveCount = Team_CountAlivePlayers(TEAM_2_INS);
		PrintToServer("insTeamAliveCount: %d", insTeamAliveCount);
		calcBotAlive();
		if (insTeamAliveCount < g_bots_spawn_max)
		{
			//Lets validate all bots (including players, we may place bots in an array to reduce loop cost)
			for (new clientBot = 1; clientBot <= MaxClients; clientBot++)
			{
				insTeamAliveCount = Team_CountAlivePlayers(TEAM_2_INS);
				PrintToServer("DEBUG 1");

				if (insTeamAliveCount > g_bots_spawn_max)
					continue;
				if (!IsValidClient(clientBot))
					continue;
				//Skip hoooomans
				if (!IsFakeClient(clientBot))
					continue;
				//If bot spawned and alive, skip
				if (g_bot_isSpawnedAlive[clientBot])
					continue;
				//If bot is special and is already delayed respawning, skip
				if (g_botSpecial_isSpawning[clientBot])
					continue;

				PrintToServer("DEBUG 2");
				//Always a good check for whatever reason	
				new team = GetClientTeam(clientBot);

				bool tCanBotSpawn = canBotClassSpawn(clientBot);

				// Check enables
				if (g_iCvar_respawn_enable && tCanBotSpawn)
				{
					...
					if (team == TEAM_2_INS && g_respawn_mode_team_ins)
					{
						...
						if (g_isCheckpoint && Ins_InCounterAttack() && 
							(((acp+1) == ncp && g_iCvar_final_counterattack_type == 2) || 
							((acp+1) != ncp && g_iCvar_counterattack_type == 2))
						)
						{
							//Bots spawn using individual lives
							if ((g_iCvar_respawn_type_team_ins == 1 && team == TEAM_2_INS))
							{
								if ((g_iSpawnTokens[clientBot] < g_iRespawnCount[team]))
									g_iSpawnTokens[clientBot] = (g_iRespawnCount[team] + 1);
								PrintToServer("WAVE SPAWNING 1!!!!!!");
								// Call respawn timer
								CreateBotRespawnTimer(clientBot);
							}
							//Bots spawn using team pool of lives
						...
						{
							if (g_iSpawnTokens[clientBot] > 0)
							{
								if (team == TEAM_2_INS)
								{
									PrintToServer("WAVE SPAWNING 3!!!!!!");
									CreateBotRespawnTimer(clientBot);
								}
							}
						}
...
//Calculate bots alive
void calcBotAlive() {
	int tBombers, tJugs, tStds;
	for (new clientBot = 1; clientBot <= MaxClients; clientBot++)
	{
		if (!IsValidClient(clientBot))
			continue;
		new team = GetClientTeam(clientBot);
		if (team == TEAM_2_INS && IsFakeClient(clientBot))
		{
			if (StrContains(g_client_last_classstring[clientBot], "bomber") > -1)
			{
				if (IsPlayerAlive(clientBot))
					tBombers++;
				else
					tBombers--;
				...
				if (tBombers <= 0)
					tBombers = 0;
				if (tBombers > g_BotBombMax)
					tBombers = g_BotBombMax;
			}
			else if (StrContains(g_client_last_classstring[clientBot], "juggernaut") > -1)
			{
				if (IsPlayerAlive(clientBot))
					tJugs++;
				else
				...
				if (tJugs <= 0)
					tJugs = 0;
				if (tJugs > g_BotJuggMax)
					tJugs = g_BotJuggMax;
			}
			else if ((StrContains(g_client_last_classstring[clientBot], "standard") > -1) || 
					(StrContains(g_client_last_classstring[clientBot], "fighter") > -1)  || 
					(StrContains(g_client_last_classstring[clientBot], "striker") > -1)  || 
					(StrContains(g_client_last_classstring[clientBot], "sapper") > -1)  || 
					(StrContains(g_client_last_classstring[clientBot], "scout") > -1))
			{
				if (IsPlayerAlive(clientBot))
					tStds++;
				else
				...
				if (tStds <= 0)
					tStds = 0;
				if (tStds > g_BotStdMax)
					tStds = g_BotStdMax;
			}
			g_BotJuggerAliveCnt = tJugs;
			g_BotBomberAliveCnt = tBombers;
			g_BotStdAliveCnt = tStds;
		}
	}
}

//int getBotClassType(client) {
...
bool canBotClassSpawn(clientBot) {

	//This is our version of ins_bots_min - ins_bots_max
	if (StrContains(g_client_last_classstring[clientBot], "bomber") > -1)
	{
		if (g_BotBomberAliveCnt < g_BotBombMax && g_BotBomberAliveCnt < g_bots_bomb)
			return true;
		else
			return false;
	}
	...
	{
		if (g_BotJuggerAliveCnt < g_BotJuggMax && g_BotJuggerAliveCnt < g_bots_jug)
			return true;
		else
			return false;
	}
	...
	{
		if (g_BotStdAliveCnt < g_BotStdMax && g_BotStdAliveCnt < g_bots_std)
			return true;
		else
			return false;
	}
		...
		// {
			...
			for (new client = 1; client <= MaxClients; client++)
			{
				if (client > 0 && IsClientInGame(client))
				{
					...
					{
						PrintToServer("REINFORCE SPAWNING 1!!!!!!");
						g_iRemaining_lives_team_ins++;
	...
	else
	{
		...
		for (new client = 1; client <= MaxClients; client++)
		{
			if (client > 0 && IsClientInGame(client))
			{
				...
				if (IsFakeClient(client) && !IsPlayerAlive(client) && m_iTeam == TEAM_2_INS)
				{
					PrintToServer("REINFORCE SPAWNING 2!!!!!!");
					g_iRemaining_lives_team_ins++;
					CreateBotRespawnTimer(client);
				}
			}
			...
			if (g_iRemaining_lives_team_ins > 0)
			{
				...
				if (g_iRemaining_lives_team_ins < (g_iRespawn_lives_team_ins / g_iReinforce_Mult) + g_iReinforce_Mult_Base)
				{
					...
					for (new client = 1; client <= MaxClients; client++)
					{
						...
						{
							//If bot spawned and alive, skip
							if (g_bot_isSpawnedAlive[client])
								continue;
							//If bot is special and is already delayed respawning, skip
							if (g_botSpecial_isSpawning[client])
								continue;

							bool tCanBotSpawn = canBotClassSpawn(client);

							new m_iTeam = GetClientTeam(client);
							if (m_iTeam == TEAM_2_INS && tCanBotSpawn)
							{
								PrintToServer("REINFORCE SPAWNING 3!!!!!!");
								g_iRemaining_lives_team_ins++;
								CreateBotRespawnTimer(client);
							}
						}
			...
			else
			{
				...
				for (new client = 1; client <= MaxClients; client++)
				{
					if (client > 0 && IsClientInGame(client) && IsFakeClient(client))
					{
						...
						if (m_iTeam == TEAM_2_INS && tCanBotSpawn)
						{
								PrintToServer("REINFORCE SPAWNING 4!!!!!!");
							g_iRemaining_lives_team_ins++;
							CreateBotRespawnTimer(client);
						}
					}
...
public Action:Event_SpawnPre(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	
	//Bot status is 1 because he alive
	if(IsFakeClient(client))
	{
		g_bot_isSpawnedAlive[client] = true;
		if (g_botSpecial_isSpawning[client])
			g_botSpecial_isSpawning[client] = false;

		//calcBotAlive(client);
		
		int insTeamAliveCount = Team_CountAlivePlayers(TEAM_2_INS);
		//Kill bots over spawning on capture
		if (insTeamAliveCount > g_bots_spawn_max)
		{
			g_bot_OverFlowKilled[client] = true;
			dy_ForceBotSuicide(client);
		}
...
//Basically force suicide for bots but lets place all bots at 0,0,0 first.
void dy_ForceBotSuicide(enemyBot)
{

	new Float:tempOrigin[3];
	//TeleportEntity(enemyBot, tempOrigin, NULL_VECTOR, NULL_VECTOR);
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//Bot status is 0 because he deadz
	if(IsFakeClient(client))
	{
		g_bot_isSpawnedAlive[client] = false;
		//Check if bot has isSpawning flag and disable on death.
		if (g_botSpecial_isSpawning[client])
			g_botSpecial_isSpawning[client] = false;
		//calcBotAlive(client);
	}
	...
	if (g_iCvar_respawn_enable)
	{
		...
		if ((team == TEAM_1_SEC && g_respawn_mode_team_sec) || (team == TEAM_2_INS && !g_respawn_mode_team_ins))
		{
			PrintToServer("SPAWN IS FIRING OFF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			// The number of control points
			new ncp = Ins_ObjectiveResource_GetProp("m_iNumControlPoints");
			...
			(StrContains(g_client_last_classstring[client], "juggernaut") > -1)))) //make sure its a bot bomber
	{
		...
		if (g_cqc_map_enabled == 1 && Ins_InCounterAttack())
		{
			if (StrContains(g_client_last_classstring[client], "bomber") > -1)
			{
				...
				g_botSpecial_isSpawning[client] = true;
